...
- **CU-01 a CU-04** (crear elección, padrón/árbol de Merkle, autoridades, checkpoints — todo Admin/Web) implementados a nivel de aplicación en `themis-core` (`src/modules/elections/`) + `themis-web`: formularios, validación, persistencia en Postgres, con tests. **Sin capa criptográfica real todavía**: no hay árbol de Merkle real, las autoridades son cuentas de plataforma (no claves), y el mecanismo/umbral multisig siguen siendo constantes fijas de código, no columnas configurables. Ver `docs/modelo-bd-registro.md` para el detalle de qué es constante vs. columna.
...
- **CU-06 en adelante** (monitoreo de ritmo, cierre de checkpoint, aprobación multisig de lotes, inserción real en el árbol on-chain, voto, resultados, auditoría) **no implementado**. El contrato on-chain (`themis-core/contracts/ThemisRegistry.sol`) sigue siendo puro andamiaje (`version()`/`ping()`), sin Semaphore integrado — nada está on-chain todavía.
