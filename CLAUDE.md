...
- **CU-10 a CU-15** (voto, conteo en vivo, conteo final, auditoría) implementados de punta a punta, con criptografía real: módulo `voting/` (`src/modules/voting/`) — `SubmitVoteUseCase` valida la prueba zk-SNARK on-chain (`Semaphore.validateProof`, mismo contrato que CU-09), `SyncVoteEventsUseCase`/`VotingScheduler` sincronizan eventos `ProofValidated` como red de seguridad, `ExecuteFinalCountUseCase` snapshotea el resultado final inmutable al cerrar. `themis-web` tiene la ruta `/prove` (pública, no linkeada) que genera la prueba con snarkjs real; `themis-app` tiene `VoteProofBridge` + `BallotPage`/`ElectionSelectPage`. Verificado end-to-end contra Hardhat local real vía `scripts/manual-test-full-flow.ts` (incluye rechazo real de doble voto por nullifier). Ver `docs/checkpoint-lote-multisig.md` sección "Fase 2: RESUELTA" para el detalle de las decisiones de diseño (scope/message de Semaphore, etc.). `ThemisRegistry.sol` (distinto de `ThemisSemaphoreRegistry.sol`) sigue siendo solo andamiaje, no participa de nada real. **Fuera de esta iteración**: CU-12/CU-13 (proyección con IA, `themis-ai`), explícitamente no tocado.
- El cliente OpenAPI generado en `themis-app/lib/data/api/` sigue vacío (los repos HTTP de la app están escritos a mano).
